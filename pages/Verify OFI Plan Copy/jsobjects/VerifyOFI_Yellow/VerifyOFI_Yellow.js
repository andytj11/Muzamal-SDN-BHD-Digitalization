export default {
  ...
  _latestRow(stage, role, onlyComment=false){
    const rows=(this.signoffs()||[]).filter(r =>
      this._norm(r.stage)===this._norm(stage) &&
      this._norm(r.role)===this._norm(role) &&
  ...
  /* ======================== YELLOW (clone of black) ======================== */
  isYellowReviewerApproved(){
    return (this._latestRow("yellow","reviewer").decision||"").toLowerCase()==="approved";
  },
  isYellowApproverApproved(){
    return (this._latestRow("yellow","approver").decision||"").toLowerCase()==="approved";
  },
  isYellowApproved(role){
    return (this._latestRow("yellow", role).decision || "").toLowerCase() === "approved";
  },

  yellowLatestComment(role){
    return this._latestRow("yellow", role, true).comment || "";
  },
  yellowLatestCommentDate(role, fmt="YYYY-MM-DD HH:mm:ss"){
    const row = this._latestRow("yellow", role, true);
    const m = moment(row.commented_at || row.created_at);
    return m.isValid() ? m.format(fmt) : "";
  },

  // Use this for the Visible property of ALL yellow widgets per role
  yellowControlsVisible(role){ return !this.isYellowApproved(role); },
  ...
  /* -------------------- Actions in yellow box (AUTH ENFORCED; exact clone) -------------------- */
  _pwWidgetYellow(role){
    const map = {
      reviewer:  (typeof YellowReviewerPassword !== "undefined") ? YellowReviewerPassword : undefined,
      approver:  (typeof YellowApproverPassword !== "undefined") ? YellowApproverPassword : undefined
    };
    return map[role];
  },
  _commentWidgetYellow(role){
    const map = {
      reviewer: (typeof YellowReviewerComment !== "undefined") ? YellowReviewerComment : undefined,
      approver: (typeof YellowApproverComment !== "undefined") ? YellowApproverComment : undefined
    };
    return map[role];
  },
  _enteredYellowPassword(role){
    const w = this._pwWidgetYellow(role);
    return w ? (w.text || "").trim() : "";
  },
  _resetYellowInputs(role){
    try {
      const pw = this._pwWidgetYellow(role);
      if (pw?.widgetName) resetWidget(pw.widgetName, true);
      const cmt = this._commentWidgetYellow(role);
      if (cmt?.widgetName) resetWidget(cmt.widgetName, true);
    } catch(_) {}
  },
  async _requireYellowAuth(role){
    const pw = this._enteredYellowPassword(role);
    if (!pw) { showAlert("Please enter your password.", "warning"); return false; }
  ...
  async approveYellow(role){
    ...
    // Password check
    const authed = await this._requireYellowAuth(role);
    if (!authed) return;
    ...
    this._resetYellowInputs(role);
    await Approvals_GetByForm.run({ form_id: fid });   // <-- make sure latest yellow is fetched
    await this.syncHeaderStatus();
    showAlert(`${_.startCase(role)} approved OFI evidence.`,"success");
  },

  async commentYellow(role, commentText, resetWidgetName){
    const fid=this.getFormId();
    if(!fid){ showAlert("No observation loaded.","error"); return; }
  ...
  computedStatus(){
    ...
    if (this.isYellowReviewerApproved() && this.isYellowApproverApproved()) return "OFI EVIDENCE APPROVED";
    // Next: Black (plan) both approved
    if (this.isReviewerApproved() && this.isApproverApproved()) return "OFI PLAN APPROVED";
    // Next: Blue attention approved
    if (this.blueAttentionApproved()) return "OFI APPROVED";
    return "OPEN";
  },
  ...
  statusForHeader(){
    ...
    if (code === "OFI EVIDENCE APPROVED") return "OFI EVIDENCE APPROVED";
    if (code === "OFI PLAN APPROVED")     return "OFI PLAN APPROVED";
    if (code === "OFI APPROVED")          return "OFI APPROVED";
    return "OPEN";
  },
